...
def string_generator(length):
    letters = string.ascii_lowercase + str(string.digits)
    return ''.join(random.choice(letters) for i in range(length))

def list_generator(min, max, n_strings):
    generated = []
    for i in range(n_strings):
        print(i)
        length = random.choice(range(min,max))
        generated.append(string_generator(length))
    return generated
